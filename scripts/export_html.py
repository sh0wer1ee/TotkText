    ...
    if region == "JPja":
        ...
        if len(rubyList):
            for i in range(len(rubyList)):
                ...
                ruby = rubyList[i][2]
                proc = proc.replace(
                    f"<Ruby={{{rubyList[i][0]}:{rubyList[i][1]}}}{ruby}>{kanji}",
...
def json2htmlsheetblock(fdr: str, filename: str) -> str:
    ...
    temp_dict = {}
    html_sheetblock += (
        f'<caption class="diy_caption">{filename.split(".")[0]}</caption>'
    )
    # index
    html_sheetblock += "<tr>"
...
def genFileList():
    filelist = {}
    for folder in os.listdir("json/totk100/CNzh.product.100/"):
        for f in os.listdir(f"json/totk100/CNzh.product.100/{folder}"):
            try:
                filelist[folder].append(f)
            except KeyError:
                filelist[folder] = [f]
    json.dump(filelist, open("filelist.json", "w"), indent=2)
    ...
    for fdr in folder:
        ...
        for r in region:
            ...
            for f in tqdm.tqdm(os.listdir(json_dir)):
                ...
                if filename not in outJson:
                    outJson[filename] = {}
                data = json.load(open(f"{json_dir}/{f}", "r", encoding="utf-16"))
                for key in data:
                    if key not in outJson[filename]:
                        outJson[filename][key] = {}
                    outJson[filename][key][r] = string2html(data[key], r)
        json.dump(
            outJson,
            open(f"{fdr}.json", "w", encoding="utf-8"),
            ensure_ascii=False,
        )
