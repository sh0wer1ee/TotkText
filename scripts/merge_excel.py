...
import tqdm
from msbt import *
from export_html import string2html
...
def write2md(json_path, md_path):
    markdown = open(md_path, "w", encoding="utf-8")
    # folder - filename - textkey - region
    for fdr in folder:
        markdown.write(f"# {fdr}\n")
        for f in tqdm.tqdm(
            os.listdir(f"json/totk{version}/CNzh.Product.{version}/{fdr}")
        ):
            markdown.write(f"## {f}\n")
            markdown.write(("|id|简中|繁中|日语|英语|\n"))
            markdown.write("|-|--|--|--|--|\n")
            zh_cn = json.load(
                open(
                    f"json/totk{version}/CNzh.Product.{version}/{fdr}/{f}",
                    "r",
                    encoding="utf-16",
                )
            )
            zh_tw = json.load(
                open(
                    f"json/totk{version}/TWzh.Product.{version}/{fdr}/{f}",
                    "r",
                    encoding="utf-16",
                )
            )
            ja_jp = json.load(
                open(
                    f"json/totk{version}/JPja.Product.{version}/{fdr}/{f}",
                    "r",
                    encoding="utf-16",
                )
            )
            en_us = json.load(
                open(
                    f"json/totk{version}/USen.Product.{version}/{fdr}/{f}",
                    "r",
                    encoding="utf-16",
                )
            )
            for key in zh_cn:
                zhcn = string2html(zh_cn[key], "CNzh")
                zhtw = string2html(zh_tw[key], "TWzh")
                jajp = string2html(ja_jp[key], "JPja").replace("Ruby>", "ruby>")
                enus = string2html(en_us[key], "USen")
                markdown.write(f"|{key}|{zhcn}|{zhtw}|{jajp}|{enus}|\n")
        exit()
...
# write2md("totk100_chs_cht_jp_en.json", "totk100.md")
