"""
    an extreeeeeeeeeeeeeeeeemely dirty modified version of KinTamashii's MSBTEditorPro/msbt.py:
        https://github.com/KinTamashii/MSBTEditorPro
"""
...
def parse_msbt_string(
    string, encoding
):  # Represent control codes in a readable format.
                                ...
                                match tag:
                                    case "Ruby":
                                        attr = (
                                            "{"
                                            + ":".join(
                                                [str(ord(i)) for i in attr_bytes[:2]]
                                            )
                                            + "}"
                                            + attr_bytes[2:]
                                        )
                                    case "Color":
                                        attr = "#" + "".join(
                                            [
                                                (hex(i))[2:].zfill(2)
                                                for i in attr_bytes.encode(encoding)
                                            ]
                                        )
                                        attr = Colors[attr]
                                    case _:
                                        attr = " ".join(
                                            [str(ord(i)) for i in attr_bytes]
                                        )
                                new_tag = f"<{tag}={attr}>"
                        else:
                            if attr_bytes == "":
                ...
                if "[" in cur_code:
                    if "]" in cur_code:
                        ...
                        raise UnmatchedCodeError(
                            'The control code has no end marker for the info section. "]"'
                        )
                    ...
                    if "=" in cur_code:
                        if cur_code.count('"') != 2:
                            raise UnmatchedCodeError(
                                "The attribute info is not quoted properly."
                            )
                        ...
                        match tag:
                            ...
                            case "Color":
                                attr = bytes.fromhex(
                                    attr_info[attr_info.index("#") + 1 :]
                                ).decode(encoding)
                            case _:
                                attr = "".join(
                                    chr(int(i)) for i in attr_info.split(" ")
                                )

                        compiled_code = (
                            escape_char + Tag_Bytes[tag] + chr(len(attr) << 1) + attr
                        )
...
class header:
    def __init__(self, msbt_file):
        ...
        self.byte_order = (
            "le" if self.byte_order_mark[0] > self.byte_order_mark[1] else "be"
        )
        self.int_read = reader(self.byte_order)
        self.unknown1 = self.int_read.ReadUInt16(msbt_file)
        self.encoding_byte = msbt_file.read(1)
        self.file_encoding = (
            "utf-8" if self.encoding_byte == b"\x00" else "utf-16-" + self.byte_order
        )
...
class TXT2:
    def __init__(self, msbt_file, parent, int_read):
        ...
        self.offsets = [
            int_read.ReadUInt32(msbt_file) for i in range(self.number_of_strings)
        ]

        self.Original_Strings = []
        for i in range(self.number_of_strings):
            next_offset = (
                self.start_of_strings + self.offsets[i + 1]
                if i + 1 < len(self.offsets)
                else self.start_of_strings + self.section_size
            )
            msbt_file.seek(self.start_of_strings + self.offsets[i], 0)
            result = b""
            while (
                msbt_file.tell() < next_offset
                and msbt_file.tell() < parent.header.filesize
            ):
            ...
            #
            self.Original_Strings += (
                parse_msbt_string(result, parent.header.file_encoding),
            )
...
class msbt:
    ...
    def add_label(self, name):
        ...
        nlbl.name = name
        nlbl.Index = self.txt2.Strings.index(
            nstr,
        )
    ...
    def remove_label(self, lbl):
        ...
        if hasattr(self, "atr1"):
            self.atr1.number_of_attributes -= 1
        self.txt2.Strings = (
            self.txt2.Strings[: lbl.Index] + self.txt2.Strings[lbl.Index + 1 :]
        )
        self.txt2.Original_Strings = (
            self.txt2.Original_Strings[: lbl.Index]
            + self.txt2.Original_Strings[lbl.Index + 1 :]
        )
        self.txt2.number_of_strings -= 1
    ...
    def save(self, save_dir):
        ...
        self.new_file.write(
            self.header.unknown1.to_bytes(
                2, "little" if self.header.byte_order == "le" else "big"
            )
        )
        self.new_file.write(self.header.encoding_byte)
        self.new_file.write(self.header.unknown2)
        self.new_file.write(
            self.header.number_of_sections.to_bytes(
                2, "little" if self.header.byte_order == "le" else "big"
            )
        )
        self.new_file.write(
            self.header.unknown3.to_bytes(
                2, "little" if self.header.byte_order == "le" else "big"
            )
        )
        self.new_file.write(
            self.header.filesize.to_bytes(
                4, "little" if self.header.byte_order == "le" else "big"
            )
        )
        ...
        self.new_file.write(
            file_size.to_bytes(4, "little" if self.header.byte_order == "le" else "big")
        )
    ...
    def write_LBL1(self):
        ...
        self.new_file.write(
            new_size.to_bytes(4, "little" if self.header.byte_order == "le" else "big")
        )
        self.new_file.write(self.lbl1.padding1)
        self.new_file.write(
            self.lbl1.number_of_groups.to_bytes(
                4, "little" if self.header.byte_order == "le" else "big"
            )
        )
    ...
    def write_NLI1(self):
        ...
        self.new_file.write(
            self.nli1.section_size.to_bytes(
                4, "little" if self.header.byte_order == "le" else "big"
            )
        )
    ...
    def write_ATO1(self):
        ...
        self.new_file.write(
            self.ato1.section_size.to_bytes(
                4, "little" if self.header.byte_order == "le" else "big"
            )
        )
    ...
    def write_ATR1(self):
        ...
        self.new_file.write(
            self.atr1.section_size.to_bytes(
                4, "little" if self.header.byte_order == "le" else "big"
            )
        )
        self.new_file.write(self.atr1.padding1)
        self.new_file.write(
            self.atr1.number_of_attributes.to_bytes(
                4, "little" if self.header.byte_order == "le" else "big"
            )
        )
    ...
    def write_TSY1(self):
        ...
        self.new_file.write(
            self.tsy1.section_size.to_bytes(
                4, "little" if self.header.byte_order == "le" else "big"
            )
        )
    ...
    def write_TXT2(self):
        ...
        for i in range(self.txt2.number_of_strings):
            new_size += len(
                compile_msbt_string(self.txt2.Strings[i], self.header.file_encoding)
            )
        ...
        self.new_file.write(
            self.txt2.number_of_strings.to_bytes(
                4, "little" if self.header.byte_order == "le" else "big"
            )
        )
...
def code_export_json(msbt, json_dir):
    if msbt.has_labels:
    ...
    json.dump(
        dict(zip(Labels_List, msbt.txt2.Strings)),
        open(json_dir, "w", encoding="utf-16"),
        ensure_ascii=False,
        indent=2,
    )
    ...
    with open(txt_dir, "w", encoding="utf-16") as f:
        f.write(
            ...
            + "\n".join(
                [
                    sub_row
                    for string_list in msbt.txt2.Strings
                    for sub_row in clean_string_export(string_list)
                ]
            )
        )
        # for  in :
